...
class EvidentlyDriftDetector:
    ...
    async def _run_evidently_detection(
        ...
        # Generate error/summary report if no HTML report was generated
        if html_report is None and results:
            html_report = self._generate_error_report(results)

        return results, html_report

    def _generate_error_report(self, results: List[DriftResult]) -> str:
        """Generate a simple HTML error/summary report when normal report generation fails."""
        drift_detected = sum(1 for r in results if r.detected)
        no_drift = sum(1 for r in results if not r.detected and r.drift_type != "error")
        errors = sum(1 for r in results if r.drift_type == "error")

        # Build results rows
        result_rows = ""
        for r in results:
            status_icon = "🔴" if r.detected else ("⚠️" if r.drift_type == "error" else "🟢")
            result_rows += f"""
            <tr>
                <td>{r.monitor_name}</td>
                <td>{r.drift_type}</td>
                <td>{status_icon} {"Drift Detected" if r.detected else ("Error" if r.drift_type == "error" else "No Drift")}</td>
                <td>{r.message or '-'}</td>
            </tr>"""
...
<head>
    ...
    <style>
        ...
        .stat.no-drift {{ background: #e8f5e9; color: #2e7d32; }}
        .stat.drift {{ background: #ffebee; color: #c62828; }}
        .stat.error {{ background: #fff3e0; color: #ef6c00; }}
...
<body>
    <div class="container">
        ...
        <div class="summary">
            <div class="stat no-drift"><div class="stat-value">{no_drift}</div><div class="stat-label">No Drift</div></div>
            <div class="stat drift"><div class="stat-value">{drift_detected}</div><div class="stat-label">Drift Detected</div></div>
        ...
        <table>
            <thead>
                <tr><th>Monitor</th><th>Type</th><th>Status</th><th>Message</th></tr>
            </thead>
            <tbody>
                {result_rows}
            </tbody>
        ...
        <div class="footer">
            <p>Generated by Chalk and Duster - Drift Detection Framework</p>
        </div>
    </div>
</body>
</html>"""
